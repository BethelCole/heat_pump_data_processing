...
create_year_state_equipm_summary = function(heating_equipment_dataset,
  ...
  if(grouping_detail == "equipm"){
    ...
    year_state_equipm_summary <- heating_equipment_dataset %>%
      ...
      summarize(mean_shrbtusph = weighted.mean(shrbtusph, w=weight, na.rm = TRUE),
                ...
                mean_btulp = weighted.mean(btulp, w=weight),
                mean_dollarlp = weighted.mean(dollarlp, w=weight),
                pop = sum(weight)) %>%
      ungroup() %>%
  ...
  return(year_state_equipm_summary)
}